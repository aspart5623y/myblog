    ...
    <div class="section">
        <div class="container">
            ...
            <div class="text-right py-4">
                <a href="{{ route('post.create') }}" class="btn btn-secondary px-4">Create Post</a>
            </div>
            <style>
                nav svg{
                    height: 20px;
            ...
            <div class="row blog-post mb-5">
                @foreach ($posts as $post)
                    ...
                    <div class="card col-xl-4 col-md-6 my-4 p-0">
                        <a href="{{ route('post.show', ['post' => $post->id]) }}" class="card-post">
                            ...
                            <div class="card-body p-0">
                                ...
                                <a href="{{ route('post.edit', ['post' => $post->id]) }}" class="close mx-2"><i class="far fa-edit"></i></a>
                                <div class="post-meta d-flex justify-content-between">
                                    <div class="date">{{ $post->created_at }}</div>
                                    <div class="category"><a href="javascript:void(0)">{{ $post->category->title }}</a></div>
                                </div>
                                    ...
                                    @else
                                        @if ($post->status == 'pending')
                                            <div class="my-2 text-center">
                                                <a href="{{ route('post.approve', ['post' => $post->id]) }}" class="btn btn-success"
                                                    onclick="event.preventDefault(); document.querySelector('.approve-post-form-'+{{ $post->id }}).submit();">
                                                        <i class="fas fa-check"></i> &nbsp; Approve post
                                                </a>

                                                <form action="{{ route('post.approve', ['post' => $post->id]) }}" method="POST" class="d-none approve-post-form-{{ $post->id }}">
                                                    @csrf
                                                </form>
                                            </div>
                                        @elseif ($post->status == 'approved')
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-body text-center">
                    ...
                    <a href="javascript:void(0)" class="btn btn-secondary px-4 modal_link"
                    onclick="event.preventDefault(); document.querySelector('#delete-form').submit();">Confirm</a>
        ...
        $(document).ready(function(){  
            $('.delete_modal').on('click', function(){
                ...
                $('#deletePost').on('show.bs.modal', function(){
                    $('#delete-form').attr('action', 'http://127.0.0.1:8000/post/'+ $id);
